 ...
 *
 *   UN SOLO ÁNGULO PARA TODA LA PLANTA, el que se pase por `--tilt`. El motor
 *   admite ángulo POR SEGUIDOR —es media fase 1— pero los presets no traen el
 *   ángulo de cada uno, y sacarlo exige el modelo de backtracking de
 *   cobertura-zigbee, que es el paso siguiente. Con un solo ángulo esto es una
 *   foto, no el día entero.
 *
...
function bandasQueCruza(n, m, R, cuerda, tilt) {
  ...
  for (let k = lo; k < segs.length; k++) {
    ...
    if (t > 0.001 && t < 0.999 && u >= 0 && u <= 1) {
      ...
      cruces.push({ s: t * D, banda: RPV.banda(ANT_H, cuerda, tilt, 0) });
    }
  }
...
function analizaPlanta(nombre, P) {
  ...
  /* margen de cada par UNA sola vez, y luego el umbral se barre sobre eso */
  const margenes = new Map();
  let evaluados = 0, podados = 0;
  ...
  for (let a = 0; a < motors.length; a++) {
    for (let b = a + 1; b < motors.length; b++) {
      ...
      evaluados++;
      const { D, cruces } = bandasQueCruza(motors[a], motors[b], R, cuerda, TILT);
      const p = ZB.presupuesto({ D, zA: ANT_H, zB: ANT_H, cruces }, PRO, PROP, null);
      margenes.set(ML.clave(motors[a].id, motors[b].id), { m: p.margenDb, n: p.mesas });
    }
  }
  ...
  for (const u of UMBRALES) {
    ...
    for (let a = 0; a < motors.length; a++) {
      for (let b = a + 1; b < motors.length; b++) {
        const r = margenes.get(ML.clave(motors[a].id, motors[b].id));
        if (!r || r.m == null || r.m < u) continue;
...
console.log('supuestos: terreno LLANO, UN solo angulo (' + TILT + ' grados) para toda la planta,');
console.log('           sin calibracion, poda geometrica a ' + ALCANCE + ' m.');
...
for (const p of informe) {
  ...
  console.log('   pares evaluados ' + p.evaluados + ' (podados ' + p.podados + ') en ' + p.ms + ' ms');
...
if (SALIDA) {
  fs.writeFileSync(SALIDA, JSON.stringify({
    ...
    supuestos: { terreno: 'llano', tilt_deg: TILT, modo: 'TEORICO', variante: 'zigbee_pro_24',
